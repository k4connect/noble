...
var NobleBindings = function() {
  ...
  this._passkeyProvider = null;

  this._ourPeripheralAddresses = new Set();
};
...
NobleBindings.prototype.connect = function(peripheralUuid) {
  ...
  this._ourPeripheralAddresses.add(address);
...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  if(!this._ourPeripheralAddresses.has(address)) return; // k4 - if noble didn't initiate the connection, noble doesn't handle this connection
  ...
  if (status === 0) {
    ...
    var aclStream = new AclStream(this._hci, handle, this._hci.addressType, this._hci.address, addressType, address, this._passkeyProvider);
...
NobleBindings.prototype.onLeConnUpdateComplete = function(handle, interval, latency, supervisionTimeout) {
  if(!this._ourPeripheralAddresses.has(this._handles[handle])) return; // k4 - If noble didn't initiate the connection, noble doesn't handle it
  // no-op
};
...
NobleBindings.prototype.onServicesDiscovered = function(address, serviceUuids) {
  if(!this._ourPeripheralAddresses.has(address)) return; // k4 - If noble didn't initiate the connection, noble doesn't handle it
...
NobleBindings.prototype.onConnectionParameterUpdateRequest = function(handle, minInterval, maxInterval, latency, supervisionTimeout) {
  if(!this._ourPeripheralAddresses.has(this._handles[handle])) return; // k4 - If noble didn't initiate the connection, noble doesn't handle connection parameter update requests
  this._hci.connUpdateLe(handle, minInterval, maxInterval, latency, supervisionTimeout);
};

NobleBindings.prototype.registerPasskeyProvider = function(passkeyProvider) {
